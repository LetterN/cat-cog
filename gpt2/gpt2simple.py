# stolen from the repo name, runs on tf-2 (heh)
...
def start_tf_sess(threads=-1, server=None):
    ...
    """
    config_v1 = tf.compat.v1.ConfigProto()
    gpu_ops_v1 = tf.compat.v1.GPUOptions()
    gpu_ops_v1.allow_growth = True
    graph_ops_v1 = tf.compat.v1.GraphOptions()
    rewriter_ops_v1 = rewriter_config_pb2.RewriterConfig()
    rewriter_ops_v1.layout_optimizer = False
    graph_ops_v1.rewrite_options = rewriter_ops_v1

    config_v1.gpu_options = gpu_ops_v1
    config_v1.graph_options = graph_ops_v1
    if threads > 0:
        config_v1.intra_op_parallelism_threads = threads
        config_v1.inter_op_parallelism_threads = threads

    if server is not None:
        return tf.compat.v1.Session(server.target, config=config_v1)
    
    return tf.compat.v1.Session(config=config_v1)
